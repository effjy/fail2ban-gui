...
static char       g_icon[64] = "fail2ban-gui";
static char       g_busname[64];
static guint      g_menu_revision = 1;

static const char *SHOW_LABEL = "Show Fail2ban GUI";
static const char *QUIT_LABEL = "Quit";
...
static void fill_props(GVariantBuilder *b, int id) {
    ...
    const char *label = (id == ID_SHOW) ? SHOW_LABEL
                      : (id == ID_QUIT) ? QUIT_LABEL : "";
...
static void menu_method(GDBusConnection *, const char *, const char *,
    ...
    if (g_strcmp0(method, "GetLayout") == 0) {
        ...
        if (g_strcmp0(name, "label") == 0)
            val = g_variant_new_string(id == ID_SHOW ? SHOW_LABEL
                                     : id == ID_QUIT ? QUIT_LABEL : "");
...
static GVariant *sni_get_prop(GDBusConnection *, const char *, const char *,
    ...
    if (g_strcmp0(prop, "Id") == 0)            return g_variant_new_string("fail2ban-gui");
    if (g_strcmp0(prop, "Title") == 0)         return g_variant_new_string("Fail2ban GUI");
    ...
    if (g_strcmp0(prop, "ToolTip") == 0) {
        ...
        return g_variant_new("(sa(iiay)ss)", g_icon, &icon, "Fail2ban GUI",
